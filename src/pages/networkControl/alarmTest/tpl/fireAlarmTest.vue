<template>
  <div class="clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
    ...
    <DashBtn>
      <div>
        <a-button type="dashed" @click="jumpRecord">故障记录</a-button>
      </div>
    </DashBtn>
      ...
      <SearchTerm>
        <a-form-model layout="inline" :model="formInlineDetail" :colon="false">
          <a-form-model-item label="区域">
            <a-input v-model="formInlineDetail.area" placeholder="请输入区域"></a-input>
          </a-form-model-item>
          <a-form-model-item label="楼层">
            <a-input v-model="formInlineDetail.floor" placeholder="请输入楼层"></a-input>
          </a-form-model-item>
          ...
          <a-form-model-item label="设备类型">
            <a-select v-model="formInlineDetail.equipType" placeholder="请选择设备类型">
...
import { preAlarmPageList, preAlarmDelete, preAlarmEdit, fireEquipPageList, removeEquip, retestEquip } from "@/services/api.js";
...
export default {
  ...
  methods: {
    initConfigPage(){
      ...
      this.statusList = dictionary("alarmStatus"); // 1：待报警  2：已报警  3：报警异常
    },
    thing(type,record){
      ...
      this.$antConfirm({
        ...
        onOk: () => {
          if(type == 1){
            ...
            let params = { type: 1, preAlarmId: this.currentDetailMsg.preAlarmId, equipFireId: record.equipFireId }
    ...
    //跳转故障记录页面
    jumpRecord(){
      this.$router.push({path: "/safeManage/emergencyManagement/networkControl/errorRecording"});
    },
    getDataList() {
      let params = {
        ...this.preFormInline,
    ...
    alarmTest(record) {
      this.alarmTestVisible = true;
      this.resetTest = true;
      this.equipFireId = record.equipFireId;
    ...
    alarmTestConfirm() {
      ...
      if (this.resetTest) { // 测试设备-重新测试
        ...
        params.equipFireId = this.equipFireId;
