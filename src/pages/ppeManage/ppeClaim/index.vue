...
export default {
  ...
  created() {
    this.columns.splice(1, 0, this.addCommonColumnItem(130, true));
  ...
  methods: {
    ...
    goHandleOrShowPge(type, record) {
      let path = type == 'handle' ? "/occupationHealth/ppeClaimHandle" : "/occupationHealth/ppeClaimShow"
      let query = { receiveId: record.receiveId };
      this.$router.push({
        path,
        query,
      });
    ...
    goDraft() {
      this.$router.push({
        path: "/occupationHealth/ppeClaimDraft",
      });
    },
    // 删除
    delDataList(record) {
