...
<script>
import { ppeStockHandleRejectApi, ppeStockHandlePassApi, ppeStockHandleSaveFilingApi } from "@/services/chemicalIdentSafetyTipsAdd.js";
...
export default {
  ...
  props: ['handleModelShow', 'modelType', 'plainOptions', 'scrapId', 'nextInfoStatus', 'detailObj', 'nextInfoStatusName'],
  data() {
    return {
      labelCol: { span: 5 },
      wrapperCol: { span: 19 },
  ...
  methods: {
    ...
    passApiFn(taskId) {
      ...
      return ppeStockHandlePassApi(apiData)
        .then((res) => {
          if (this.isEnd) {
            // 结束归档api
            this.saveFilingFn()
    ...
    // 代办推送
    async pushTask() {
      const url = process.env.VUE_APP_LOGIN_URL + "client_id=" + process.env.VUE_APP_CLIENTID + "&response_type=" + process.env.VUE_APP_RESPONSE_TYPE + "&redirect_uri=" + process.env.VUE_APP_REDIRECT_URI + "&routeUrl=" + `/occupationHealth/ppeStockHandle&scrapId=${this.scrapId}`
    ...
    saveFilingFn() {
      ...
      return ppeStockHandleSaveFilingApi(apiData)
        .then((res) => {
    ...
    rejectApiFn() {
      ...
      return ppeStockHandleRejectApi(apiData)
        .then((res) => {
          this.closeModel()
          this.$antMessage.success(`${this.modelTitle}成功`);
          // 跳转草稿箱列表页
