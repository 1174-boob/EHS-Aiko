        ...
        <HasFixedBottomWrapper>
          <a-form-model ref="ruleForm" :model="iFrom" :rules="iRules" :label-col="labelCol" :wrapper-col="wrapperCol">
            <template title="基本信息">
              ...
              <a-row>
                <a-col :span="13">
                  <staffOrDept :labelTitle="'负责人'" :checkAbel="false" :treeRoles="iRules" :propKey="'principalUserId'" :checkedTreeNode="checkedTreeNode" :onPreview="true"></staffOrDept>
                </a-col>
              </a-row>
              <a-row>
                <a-col :span="13">
                  <a-form-model-item ref="principalUserPhone" label="手机号" prop="principalUserPhone">
                    <a-input disabled v-model.trim="iFrom.principalUserPhone" :maxLength="30" placeholder="请输入手机号"></a-input>
                  </a-form-model-item>
                </a-col>
              </a-row>
            </template>
...
export default {
  components: { FixedBottom, UploadBtnStyle, OrganizeLazyTree, OperationLog, HandleModel, staffOrDept },
  ...
  data() {
    return {
      ...
      iRules: {
        organizationId: [{ required: true, message: "所属组织不能为空", trigger: "change" },],
  ...
  created() {
    ...
    // 主键id
    this.scrapId = this.$route.query.scrapId || getQueryVariable("scrapId")
    // 流程key
  ...
  computed: {
    // 当前页面是否为处理
    isHandlePage() {
      return this.$route.path.indexOf('ppeStockHandle') != -1
    },
    // 是否为最后一个节点
    isEnd() {
  ...
  methods: {
    ...
    getPpeClaimLisNoPaget() {
      const params = {
        organizationId: this.iFrom.organizationId,
    ...
    // 获取页面详情
    getPageDetail() {
      let scrapId = this.$route.query.scrapId || getQueryVariable("scrapId")
      let apiData = { scrapId }
