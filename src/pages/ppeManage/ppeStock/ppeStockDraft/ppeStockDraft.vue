<template>
  <div class="searchtable-wrapper clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
    <PageTitle>PPE报废管理草稿箱</PageTitle>
    <SearchTerm>
    ...
    <CommonTable :spinning="tableSpinning" :page="page" :pageNoChange="pageNoChange" :showSizeChange="showSizeChange">
      ...
      >
        <div slot="action" slot-scope="text,record">
          <span class="color-0067cc cursor-pointer" @click="goChangePage(record)">编辑</span>
...
import { getOrganizeList, GetFirstNode, CreateProcess, GetNextTask, ApprovedTask, GetNodeDetail, DeptUserTree } from '@/services/api'
...
export default {
  ...
  methods: {
    ...
    getTreeData(value) {
      ...
      this.formInline.principalUserId = userId;
    },
    // 获取列表
    getTableList() {
    ...
    delDataList(record) {
      this.$antConfirm({
        title: "确定删除吗?",
        onOk: () => {
          return rmPpeStockItemApi({ scrapId: record.scrapId })
            .then((res) => {
