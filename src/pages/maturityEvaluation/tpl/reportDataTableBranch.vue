  ...
  <div>
    <div class="flex justify-content-between align-items-center">
      ...
      <div class="m-b-20 color-red text-right font-18 font-bold">
        <span v-if="startStatus==2" class="m-r-15">分档：{{reportLevel ? reportLevel : "--"}}</span>
        <a-button v-if="!hideDownLoad" type="primary" @click="downLoadReport">下载</a-button>
      </div>
    </div>
    <vxe-table
    ...
    >
      ...
      <vxe-column field="weidu" title="维度" width="100">
        <template #default="{ row }">维度1234</template>
      </vxe-column>
      <vxe-column field="shelimudi" title="设立目的" min-width="100">
        <template #default="{ row }">设立目的</template>
      </vxe-column>
      <vxe-column field="projectId" title="项目" width="100">
        <template #default="{ row }">{{getMappingValue(projectList, "key", row.projectId).value}}</template>
      </vxe-column>
      <vxe-column field="dingyi" title="定义" min-width="100">
        <template #default="{ row }">定义</template>
      </vxe-column>
      <vxe-column field="fenzhi" title="分值" width="100">
        <template #default="{ row }">分值</template>
      </vxe-column>
      <vxe-column field="defen" title="得分" width="100">
        <template #default="{ row }">得分</template>
      </vxe-column>
      <vxe-column field="jisuanmingxi" title="计算明细" min-width="150">
        <template #default="{ row }">
          {{row.jisuanmingxi}}
        </template>
      </vxe-column>
      <vxe-column field="sitesNum" title="现场情况" width="100">
        <template #default="{ row }">
          <div class="flex align-items-center">
            <vxe-input :disabled="type == 'view'" width="80" type="number" :min="0" v-model="row.sitesNum" placeholder="请点击输入现场情况" allowClear @change="(e) => {sitesNumChange(e, row)}"></vxe-input>
            {{row.unit}}
          </div>
        </template>
      </vxe-column>
      ...
      <vxe-column field="wentidiantupian" title="问题点图片" width="100">
        <template #default="{ row }">
          <div class="flex align-items-center">
           问题点图片
          </div>
      ...
      <vxe-column field="shujulaiyuan" title="数据来源" width="100">
        <template #default="{ row }">
          <div class="flex align-items-center">
           数据来源
          </div>
      ...
      <vxe-column field="remake" title="备注" width="100"></vxe-column>
    </vxe-table>
...
export default {
  ...
  methods: {
    rowspanMethod: rowspanMethod(['weidu','projectId','fenzhi','defen','remake']),
    BigNumber,
