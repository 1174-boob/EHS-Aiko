  ...
  <div class="clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
    ...
    <CommonTable :spinning="tableSpinning" :page="page" :pageNoChange="pageNoChange" :showSizeChange="showSizeChange">
      <vxe-table class="vxe-scrollbar beauty-scroll-fireBox" border show-header-overflow show-overflow align="center" :row-config="{isHover: true}" :data="tableList">
        ...
        <vxe-column field="warningReason" :min-width="140" title="预警描述"></vxe-column>
        <vxe-column field="alarmDown" :min-width="150" title="下限预警值(mg/L)"></vxe-column>
        <vxe-column field="alarmUp" :min-width="150" title="上限预警值(mg/L)"></vxe-column>
        ...
        <vxe-column field="alarmDuration" :min-width="150" title="预警持续时间(小时)"></vxe-column>
...
import { getEarlyWarnInfoWasteWaterListApi, exportEarlyWarnInfoWasteWaterItemApi, getEarlyWarnInfoWasteWaterListTotalApi } from "@/services/earlyWarnAlarmInfo.js";
...
export default {
  ...
  methods: {
    ...
    // 批量导出
    exportExcel() {
      if (!this.canClickBtnMixin("earlyWarnInfoWaterExport")) {
        return;
      }
      ...
      exportEarlyWarnInfoWasteWaterItemApi(apiData)
        .then(res => {
          this.spreadSheetExcel(res, '废水预警信息')
        })
        .catch(() => { })
        .finally(() => {
    ...
    goShowOrHandlePage(type, row) {
      ...
      let path = type == 'show' ? '/environmentManage/environmental/wasteWaterShow' : '/environmentManage/environmental/wasteWaterHandle'
      this.$router.push({
        path,
        query,
      });
