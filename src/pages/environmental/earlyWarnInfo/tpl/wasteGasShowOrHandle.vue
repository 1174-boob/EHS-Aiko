      ...
      <a-spin :spinning="spinning" wrapperClassName="a-spin">
        <HasFixedBottomWrapper>
          ...
          <a-form-model ref="iFrom" :model="iFrom" :rules="iRules" :label-col="labelCol" :wrapper-col="wrapperCol" :colon="false">
            ...
            <a-form-model-item ref="warningReason" label="预警原因描述" prop="warningReason">
              <a-textarea placeholder="请输入预警原因描述" v-model="iFrom.warningReason" :maxLength="200" :disabled="isShowPage" />
            </a-form-model-item>
            <a-form-model-item ref="warningPictureList" label="预警原因描述照片" prop="warningPictureList">
...
import { getEarlyWarnInfoDetailApi, changeEarlyWarnInfoItemApi, getEarlyWarnInfoLogList } from "@/services/earlyWarnAlarmInfo.js";
...
export default {
  ...
  data() {
    return {
      ...
      // 表单验证
      iRules: {
        warningReason: [{ required: true, message: "预警原因描述不能为空", trigger: "blur" }],
        warningPictureList: [{ required: false, message: "预警原因描述照片不能为空", trigger: "change" }],
  ...
  methods: {
    ...
    //获取详情
    getDetailsFn() {
      let params = { gasId: this.gasId };
      return getEarlyWarnInfoDetailApi(params)
        .then(res => {
          if (res.data) {
            let { corporationId, warningReason, warningPictureList, dealReason, dealPictureList, dealWithType, alarmState, relieveTime, relieveValue } = res.data
            if (alarmState == 1 || alarmState == 2) {
              ...
              return this.handleDisplayPicture({ warningPictureList, dealPictureList })
                .then(res1 => {
                  ...
                  // 赋值
                  this.iFrom = { corporationId, warningReason, warningPictureList, warningPictureEchoList, dealReason, dealPictureList, dealPictureEchoList, dealWithType, alarmState, relieveTime, relieveValue };
                  console.log(this.iFrom);
                  return Promise.resolve()
                })
            } else {
              this.iFrom = { corporationId }
              return Promise.resolve()
            }
          }
        })
    ...
    //获取日志--api
    getFlowLogList() {
      let apiData = { gasId: this.gasId };
      return getEarlyWarnInfoLogList(apiData)
        .then(res => {
    ...
    handleConfirm() {
      ...
      this.handleLoading();
      changeEarlyWarnInfoItemApi(apiData)
        .then(() => {
          this.$antMessage.success("处理成功");
          this.$router.push({
            path:'/environmentManage/environmental/earlyWarnInfo'
          })
        })
        .finally(() => {
          this.cancelLoading();
