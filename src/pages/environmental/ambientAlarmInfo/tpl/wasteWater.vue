  ...
  <div class="clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
    ...
    <DashBtn class="dashBtn-box">
      <div class="pe-data-container">
        <div class="pe-data-item">
          <span class="pe-data-body">报警总数 {{listTotal.allAlarmCount}} 个</span>
        </div>
        <div class="pe-data-item">
          <span class="pe-data-body">今日报警 {{listTotal.todayCount}} 个</span>
        </div>
        <div class="pe-data-item">
          <span class="pe-data-body">当前报警 {{listTotal.unCloseCount}} 个</span>
        </div>
      </div>
      <div>
    ...
    <CommonTable :spinning="tableSpinning" :page="page" :pageNoChange="pageNoChange" :showSizeChange="showSizeChange">
      <vxe-table class="vxe-scrollbar beauty-scroll-fireBox" border show-header-overflow show-overflow align="center" :row-config="{isHover: true}" :data="tableList">
        ...
        <vxe-column field="warningTime" :min-width="150" title="报警时间"></vxe-column>
        <vxe-column field="nowValue" :min-width="140" title="实时值(mg/m3)"></vxe-column>
        <vxe-column field="alarmDown" :min-width="150" title="下限报警值(mg/L)"></vxe-column>
        <vxe-column field="alarmUp" :min-width="150" title="上限报警值(mg/L)"></vxe-column>
        <vxe-column field="categoryName" :min-width="120" title="报警类型"></vxe-column>
...
import { getEarlyWarnInfoWasteWaterListApi, exportEarlyAlarmInfoWasteWaterItemApi, getEarlyWarnInfoWasteWaterListTotalApi } from "@/services/earlyWarnAlarmInfo.js";
...
export default {
  ...
  methods: {
    ...
    getListTotal() {
      //  1预警  2报警
      let apiData = { type: '2' }
      getEarlyWarnInfoWasteWaterListTotalApi(apiData)
        .then(res => {
    ...
    getTableList() {
      ...
      this.tableSpinning = true
      getEarlyWarnInfoWasteWaterListApi(apiData)
        .then((res) => {
          ...
          tableList.forEach(item => {
            ...
            // 报警类型
            item.categoryName = item.category ? dictionary('waste_gas_reported_type', item.category) : ''
            // 报警状态
    ...
    // 批量导出
    exportExcel() {
      if (!this.canClickBtnMixin("ambientAlarmInfoWaterExport")) {
        return;
      }
      ...
      exportEarlyAlarmInfoWasteWaterItemApi(apiData)
        .then(res => {
          this.spreadSheetExcel(res, '废水报警信息')
        })
        .catch(() => { })
        .finally(() => {
    ...
    goShowOrHandlePage(type, row) {
      let query = { waterId: row.waterId };
      let path = type == 'show' ? '/environmentManage/environmental/ambientWasteWaterShow' : '/environmentManage/environmental/ambientWasteWaterHandle'
      this.$router.push({
        path,
        query,
      });
