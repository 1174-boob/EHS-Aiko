  ...
  <div class="clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
    ...
    <CommonTable :spinning="tableSpinning" :page="page" :pageNoChange="pageNoChange" :showSizeChange="showSizeChange">
      <vxe-table class="vxe-scrollbar beauty-scroll-fireBox" border show-header-overflow show-overflow align="center" :row-config="{isHover: true}" :data="tableList">
        ...
        <vxe-column field="classificationName" :min-width="120" title="报警分类"></vxe-column>
        ...
        <vxe-column field="alarmConcentration" :min-width="150" title="浓度报警值(mg/m3)"></vxe-column>
        <vxe-column field="alarmRate" :min-width="120" title="速率报警值"></vxe-column>
...
import { getEarlyWarnInfoListApi, exportEarlyAlarmInfoItemApi, getEarlyWarnInfoListTotalApi } from "@/services/earlyWarnAlarmInfo.js";
...
export default {
  ...
  created() {
    // console.log(dictionary('waste_gas_status'));
  },
  mounted() {
    this.initConfigPage()
    .finally(() => {
  ...
  methods: {
      ...
      getEarlyWarnInfoListApi(apiData)
        .then((res) => {
          ...
          tableList.forEach(item => {
            ...
            console.log('matchPollutantInfo', matchPollutantInfo);
            // 监控点
    ...
    // 批量导出
    exportExcel() {
      if (!this.canClickBtnMixin("ambientAlarmInfoGasExport")) {
        return;
      }
      ...
      exportEarlyAlarmInfoItemApi(apiData)
        .then(res => {
          this.spreadSheetExcel(res, '废气报警信息')
        })
        .catch(() => { })
        .finally(() => {
    ...
    goShowOrHandlePage(type, row) {
      ...
      let path = type == 'show' ? '/environmentManage/environmental/ambientWasteGasShow' : '/environmentManage/environmental/ambientWasteGasHandle'
      this.$router.push({
        path,
        query,
      });
