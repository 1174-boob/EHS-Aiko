...
export default {
  ...
  methods: {
    ...
    passApiFn(taskId) {
      ...
      return transientChemicalsHandlePassApi(apiData)
        .then((res) => {
          if (this.isEnd) {
            // 结束归档api
            this.saveFilingFn()
    ...
    // 代办推送
    async pushTask() {
      const url = process.env.VUE_APP_LOGIN_URL + "client_id=" + process.env.VUE_APP_CLIENTID + "&response_type=" + process.env.VUE_APP_RESPONSE_TYPE + "&redirect_uri=" + process.env.VUE_APP_REDIRECT_URI + "&routeUrl=" + `/ehsGerneralManage/cooperationPartner/transientChemicalsHandle&temporaryEntryId=${this.temporaryEntryId}`
    ...
    saveFilingFn() {
      ...
      return transientChemicalsHandleSaveFilingApi(apiData)
        .then((res) => {
    ...
    rejectApiFn() {
      ...
      return transientChemicalsHandleRejectApi(apiData)
        .then((res) => {
          this.closeModel()
          this.$antMessage.success(`${this.modelTitle}成功`);
          // 跳转草稿箱列表页
