  ...
  <HasFixedBottomWrapper>
    <div class="form-content">
      <a-form-model ref="ruleForm" :model="depotForm" :rules="depotRules" :colon="false">
        <a-row class="m-t-20">
          <a-col :span="12">
            <CommonSearchItem ref="commonSearchItem" :CommonFormInline="depotForm" :rules="depotRules" :hasDepartment="true" :notTablePage="true" :departmentMultiple="true" :labelCol="labelCol" :wrapperCol="wrapperCol" deptLabel="管理部门"></CommonSearchItem>
            ...
            <a-form-model-item label="库房名称" prop="name" :label-col="labelCol" :wrapper-col="wrapperCol">
              <a-input v-model.trim="depotForm.name" placeholder="请输入库房名称" allowClear/>
            </a-form-model-item>
            <a-form-model-item label="库房位置" prop="location" :label-col="labelCol" :wrapper-col="wrapperCol">
              <a-input v-model.trim="depotForm.location" placeholder="请输入库房位置" allowClear/>
            </a-form-model-item>
            
            <!-- <a-form-model-item label="管理人员" prop="personList" :label-col="labelCol" :wrapper-col="wrapperCol">
              <a-tree-select
                show-search
                allowClear
                multiple
                :filterTreeNode="filterUserTree"
                v-model="depotForm.personList"
            ...
            <staffOrDept :labelTitle="'管理人员'" :checkedTreeNode="checkedTreeNode" :treeRoles="depotRules" :propKey="'personList'" @getTreeData="getTreeData" :labelCol="labelCol" :wrapperCol="wrapperCol"></staffOrDept>
...
import { warehouseSave,warehouseDetail,warehouseUpdate,warehouseDeleteStorage } from "@/services/api";
...
export default {
  ...
  methods: {
    ...
    getTreeData(value) {
      ...
      this.depotForm.personList = value.treeIdList;
      this.checkedTreeNode = value.treeIdList;
      if (!formValidator.formItemValidate(this, "personList", "ruleForm")) {
        return;
      }
    },
    // 获取用户组织树
      ...
      this.$antConfirm({
        ...
        onOk() {
          if (record.warehouseId) {
            warehouseDeleteStorage({warehouseId: record.warehouseId,storageId: record.storageId}).then(res=>{
      ...
      if(this.addEdit == 'add') {
        warehouseSave(params).then(res=>{
          if(res.code == 20000) {
            ...
            this.$router.push("/safeManage/chemicalSafeManage/chemicalStoreManage/storageDepot");
          }
        }).catch(err=>{
          console.log(err);
        })
