    ...
    <SearchTerm>
      <a-form-model layout="inline" :model="formInline" :colon="false">
        ...
        <a-form-model-item label="所属分类">
          <a-select v-model="formInline.classificationCode" placeholder="请选择" show-search :filter-option="filterOptionMixin">
            <a-select-option v-for="(val,key) in classDictObj" :key="key" :value="key">{{ val }}</a-select-option>
          </a-select>
        </a-form-model-item>
...
import { GetCommonList, AllClassList } from '@/services/dangerSource/dangerList/index.js'
  ...
  methods: {
    ...
    goDangerSourceDetailsPage(record) {
      let routeUrl = this.$router.resolve({
        ...
        query: { id: record.id, type: 1, back: true }
      });
    ...
    allClassList() {
      return AllClassList()
        .then((res) => {
          ...
          console.log('获取所有所属分类', this.classDictObj);
        })
        .catch((err) => {
          console.log(err);
        });
