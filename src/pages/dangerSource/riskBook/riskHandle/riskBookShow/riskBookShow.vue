        ...
        <HasFixedBottomWrapper>
          <a-form-model ref="ruleForm" :model="iFrom" :rules="iRules" :label-col="labelCol" :wrapper-col="wrapperCol">
              ...
              <a-row>
                <a-col :span="13">
                  <a-form-model-item ref="deptId" label="部门" prop="deptId">
                    <DeptTree disabled :placeholder="iFrom.corporationId ? '请选择':'请先选择所属组织'" v-model="iFrom.deptId" :deptData="deptData"></DeptTree>
                  </a-form-model-item>
                </a-col>
              </a-row>
              <a-row>
                <a-col :span="13">
                  <a-form-model-item ref="riskPointLocation" label="所在位置" prop="riskPointLocation">
                    <a-input v-model="iFrom.riskPointLocation" disabled :maxLength="30" placeholder="请输入所在位置"></a-input>
                  </a-form-model-item>
                </a-col>
              </a-row>
              <a-row>
                <a-col :span="13">
                  <a-form-model-item ref="riskPointName" label="风险点" prop="riskPointName">
                    <a-input v-model="iFrom.riskPointName" disabled :maxLength="30" placeholder="请输入风险点"></a-input>
                  </a-form-model-item>
                </a-col>
              </a-row>
              <a-row>
                <a-col :span="13">
                  <a-form-model-item ref="riskLevel" label="风险分级" prop="riskLevel">
                    <a-select v-model="iFrom.riskLevel" placeholder="请先关联危险源" option-filter-prop="children" show-search :filter-option="filterOption" disabled @change="riskLevelChange">
            ...
            <template title="管控责任主体">
              ...
              <a-row>
                <a-col :span="13">
                  <StaffOrDept ref="staffCharge" :labelTitle="'分管安全副总'" :checkedTreeNode="checkedCharge" :onPreview="true" :checkAbel="false"></StaffOrDept>
                </a-col>
              </a-row>
            </template>
          </a-form-model>
...
import { addRiskBookApi, getRiskBookDetailApi, editRiskBookApi, getControlsLevelByRiskLevelApi } from "@/services/dangerSource/risk";
import { DeptUserTree, getLiablePeopleByDeptId } from '@/services/api'
import chemicalDict from "@/mixin/chemicalDict.js";
import cancelLoading from "@/mixin/cancelLoading";
import dictionary from "@/utils/dictionary";
import ActiveTable from '../../riskHandle/riskBookAddOrChange/components/activeTable.vue'
import StaffOrDept from "@/components/staffOrDept";
export default {
  components: { FixedBottom, ActiveTable, StaffOrDept },
  ...
  methods: {
      ...
      getLiablePeopleByDeptId(apiData)
        .then(res => {
          if (res.data && res.data.userId) {
            this.$set(this.iFrom.riskPointControlResponsibilityList[index], 'responsibilityPersonId', res.data.userId)
          } else {
    ...
    riskPointControlResponsibilityListValidator(rule, value, callback) {
      ...
      let hasPersonNull = value.some(item => {
        return item.responsibilityPersonId == undefined
      })
      if (value.length == 0) {
    ...
    saveChemicalIdentSafetyTips() {
      ...
      const apiName = this.isAddPage ? addRiskBookApi : editRiskBookApi
      this.handleLoading();
      apiName(apiData)
        .then(res => {
    ...
    onSubmitHazardSourceDrawer() {
      // 共性
      console.log('共性,', this.$refs.dangerSourceEqual.selectedRow);
      ...
      // 特异性
      console.log('特异性,', this.$refs.dangerSourceSpecial.selectedRow);
    ...
    riskLevelHandleAuto(dangerSourceList) {
      if (dangerSourceList.length) {
        ...
        console.log('处理风险等级', dangerSourceList);
      } else {
