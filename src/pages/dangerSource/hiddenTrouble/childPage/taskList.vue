<template>
  ...
  >
    <div ref="print">
            ...
            <div class="head-l-module">
              <span class="is-check-person"
                >检查人：<img
                  ...
                  class="signImg"
                /> </span
              ><span>检查时间:{{ baseInfoData.checkTime }}</span>
            </div>
            <div class="head-r-module">
              <span>待排查: {{ baseInfoData.unUseCheckCount }}</span
              ><span>应排查: {{ baseInfoData.totalCheckCount || "--" }}</span
              ><span
                >排查进度:
                {{ (baseInfoData.checkProgress || "--") + "%" }}</span
              >
            </div>
          </div>
          <div v-else>
      ...
      <!--startprintBG-->
      <div class="stakList" v-for="(itemTask, index) in dataList" :key="index">
        <div class="stak-head">
          <div>
            <a-row :gutter="16">
              <a-col :span="6">
                <div class="gutter-item gutter-item-top">
                  序号:
                  <span>{{ index + 1 }}</span>
                </div>
              </a-col>
              <a-col :span="6">
                <div class="gutter-item gutter-item-top">
                  　　部门:
                  <span>{{
                    itemTask.deptId ? deptCache[itemTask.deptId] : "--"
                  }}</span>
                </div> </a-col
              ><a-col :span="6">
                <div class="gutter-item gutter-item-top">
                  　风险点:
                  <span>{{ itemTask.riskPointName }}</span>
                </div> </a-col
              ><a-col :span="6">
                <div class="gutter-item gutter-item-top">
                  管控层级:
                  <span>{{
                    itemTask.controlLevel | systemFilter("controlLevel")
                  }}</span>
                </div> </a-col
              ><a-col :span="6">
                <div class="gutter-item">
                  编号:
                  <span>{{ itemTask.riskPointNo }}</span>
                </div>
              </a-col>
              <a-col :span="6">
                <div class="gutter-item">
                  ...
                  <span class="specific-span">{{
                    itemTask.riskLevel | systemFilter("riskLevel")
                  }}</span>
                </div> </a-col
              ><a-col :span="6">
                <div class="gutter-item">
                  ...
                  <span>{{
                    itemTask.checkFrequencyLevel
                      | systemFilter("checkFrequencyLevel")
                  }}</span>
                </div>
              </a-col>
            </a-row>
          ...
          <div class="stak-content-title">
            <a-row>
              <a-col class="stak-col" :span="4">
                <div class="gutter-title">措施分类</div>
              </a-col>
              <a-col class="stak-col" :span="4">
                <div class="gutter-title">管控措施</div>
              </a-col>
              <a-col class="stak-col" :span="4">
                <div class="gutter-title">排查标准</div>
              </a-col>
              <a-col class="stak-col" :span="4">
                <div class="gutter-title">检查结果</div>
              </a-col>
              <a-col class="stak-col" :span="4">
                <div class="gutter-title">异常简述</div>
              </a-col>
              <a-col class="stak-col" :span="4">
                <div class="gutter-title">异常关闭状态</div>
              </a-col>
            </a-row>
          </div>
          <div
          ...
          >
            ...
            >
              <a-row type="flex" justify="space-around" align="middle">
                ...
                <a-col class="stak-col" :span="4">
                  <div v-for="(item, index) in itemTask.list" :key="index">
                    <p v-if="item.type == itemMeasure.type">
                      {{ item.detailContent }}
                    </p>
                  </div>
                </a-col>
                ...
                <a-col class="stak-col" :span="4">
                  <div v-for="(item, index) in itemTask.list" :key="index">
                    <p v-if="item.type == itemMeasure.type">
                      {{
                        item.abnormalCloseStatus == 0
                          ? "未开始"
                          : item.abnormalCloseStatus == 1
                          ? "进行中"
                          : "已关闭"
                      }}
                    </p>
                  ...
                  <div v-for="(item, index) in itemTask.list" :key="index">
                    <p v-if="item.type == itemMeasure.type">
                      <a-radio-group
                        ...
                        @change="handleSeleckChange(item)"
                      />
                    </p>
                  </div>
                </a-col>
    ...
    <div class="floor">
      <div>
        <a-button class="m-l-15" @click="handleBack">返 回</a-button>
        <span></span>
        <a-button v-if="isCheck" type="primary" @click="handleDownloadPdf"
          >下 载</a-button
        >
        <a-button v-else type="primary" :loading="loading" @click="handleConfirm">确 定</a-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    initData() {
      ...
      getTaskDetail({
        ...
        if (res.data.handSignature) {
          getPortraitUrlt([res.data.handSignature]).then((res) => {
            if (res.data[0]) {
              this.imageUrl = res.data[0].filePath || null;
            }
          });
        }
    ...
    formValidate() {
      ...
      let formAll = true
      this.$refs["baseInfoData"].validate((valid, object) => {
        if (!valid) {
          formAll = false
    ...
    handleConfirm() {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        if (list[i].checkStatus == 2 && !list[i].abnormalText) {
          this.$antMessage.error(`选择为异常的，请填写异常简述！`);
          return;
        }
      }
      getTaskUpdata({
        ...
        checkTime: this.baseInfoData.checkTime
          ? dayJs(this.baseInfoData.checkTime).format("YYYY-MM-DD")
          : null,
        list: list,
      }).then((res) => {
