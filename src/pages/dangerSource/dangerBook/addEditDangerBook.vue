     ...
     <a-spin :spinning="spinning" wrapperClassName="a-spin">
      <a-form-model ref="ruleForm" :model="iFrom" :rules="iRules" :label-col="labelCol" :wrapper-col="wrapperCol">
        ...
        <template title="基本信息">
          <a-row>
            <a-col :span="12">
              <CommonSearchItem ref="commonSearchItem" :rules="iRules" :CommonFormInline="iFrom" :notTablePage="true"></CommonSearchItem>
              <a-form-model-item label="部门" prop="deptName">
                <a-input v-model="iFrom.deptName" placeholder="请输入部门" allowClear></a-input>
              </a-form-model-item>
              <a-form-model-item label="设备或区域" prop="equipmentArea">
                <a-input v-model.trim="iFrom.equipmentArea" :maxLength="100" placeholder="请输入设备或区域"></a-input>
              </a-form-model-item>
            </a-col>
            <a-col :span="12">
              <a-form-model-item label="工艺" prop="workmanship">
                <a-input v-model.trim="iFrom.workmanship" :maxLength="30" placeholder="请输入工艺"></a-input>
              </a-form-model-item>
              <a-form-model-item label="作业状态" prop="jobStatus">
                <a-select v-model="iFrom.jobStatus" placeholder="请选择">
          ...
          <a-row>
            <a-col :span="12">
              <a-form-model-item label="危险源名称" prop="riskName">
                <a-input v-model.trim="iFrom.riskName" :maxLength="30" placeholder="请输入"></a-input>
              </a-form-model-item>
            </a-col>
            <a-col :span="12">
            ...
            <a-col :span="12">
              <a-form-model-item label="危险源种类" prop="riskClass">
                <a-select v-model="iFrom.riskClass" placeholder="请选择">
            ...
            <a-col :span="12">
             <a-form-model-item label="可能接触伤害的部位" prop="partsBodyInjury">
                <a-select v-model="iFrom.partsBodyInjury" placeholder="请选择"  >
            ...
            <a-col :span="12">
              <a-form-model-item label="风险描述" prop="riskDescription">
                <a-textarea v-model.trim="iFrom.riskDescription" :maxLength="500" placeholder="请输入风险描述"></a-textarea>
              </a-form-model-item>
            </a-col>
          </a-row>
        </template>
          ...
          <a-row>
            <a-col :span="12">
               <a-form-model-item label="发生事故事件的可能性" prop="accidentsIncidents">
                <a-select v-model="iFrom.accidentsIncidents" placeholder="请选择" @change="changeAll($event,'lvalue')">
            ...
            <a-col :span="12">
               <a-form-model-item label="人员暴露于危险环境的频繁程度" prop="environment">
                <a-select v-model="iFrom.environment" placeholder="请选择" @change="changeAll($event,'evalue')">
          ...
          <a-row>
            <a-col :span="12">
               <a-form-model-item label="人员伤害" prop="personalInjury">
                <a-select v-model="iFrom.personalInjury" placeholder="请选择" @change="changeAll($event,'cvaluep')">
            ...
            <a-col :span="12">
               <a-form-model-item label="生产损失" prop="productionLoss">
                <a-select v-model="iFrom.productionLoss" placeholder="请选择" @change="changeAll($event,'cvalues')">
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    ...
    if (id) {
      next(vm => {
        vm.setRouterCode("dangerBook");
        vm.addEdit = 'edit';
        vm.dangerBookId = id;
        vm.getAccountDetail(id);
      })
    } else {
      next(vm=>{
        vm.setRouterCode("dangerBook");
  ...
  methods:{ 
    ...
    // 确定
    confirmSubmit() {
      console.log("确定。。。。",this.iFrom);
      if (!formValidator.formAll(this, "ruleForm")) {
        return;
      }
      if(this.addEdit == 'add') {
        accountSave(this.iFrom).then(res=>{
          this.$antMessage.success("新增成功");
          this.$router.push("/safeManage/dualControlManage/riskManage/dangerBook")
        }).catch(err=>{
          console.log(err);
        })
      } else {
        accountUpdate(this.iFrom).then(res=>{
