<template>
  <div class="clx-show-scroll clx-flex-1 beauty-scroll bg-fff" id="pdfDom">
    <PageTitle>管理绩效现地</PageTitle>
    <SearchTerm>
      <a-form-model layout="inline" :model="formInline" :colon="false">
        <a-form-model-item class="flex" label="所属组织">
          <a-select v-model="formInline.corporationId" placeholder="请选择所属组织">
    ...
    <DashBtn>
      <div>
        <a-button type="dashed" :loading="showPrintPdfBtn" @click="downloadPage">下载页面</a-button>
      </div>
    </DashBtn>
    <div v-if="preFormInline.corporationId && preFormInline.year && preFormInline.quarter">
      <ReportDataTableVue
        v-if="reportData && reportData.length > 0"
        ref="reportDataTableVue"
        :hideDownLoad="true"
        :reportData="reportData"
        :corporationMsg="preFormInline.corporationId ? preFormInline : undefined"
        type="view"
      ></ReportDataTableVue>
      <div class="text-center color-0067cc font-16 m-b-30" v-else>该查询条件下暂时没有填报报表数据</div>
    </div>
    <div class="text-center color-0067cc font-16 m-b-30" v-else>请先选择组织、年份、季度</div>
    <div class="m-t-20">
      <SearchTerm>
        <a-form-model layout="inline" :model="chartFormInline" :colon="false">
          <a-form-model-item class="flex" label="所属组织">
            <a-select v-model="chartFormInline.corporationId" placeholder="请选择所属组织">
              <a-select-option v-for="item in getCommonAddOrgnizeListAll" :key="item.id" :value="item.id">{{item.orgAbbrName}}</a-select-option>
            </a-select>
          </a-form-model-item>
      ...
      <div class="m-b-20 color-0067cc">必须选择组织才能展示数据</div>
      <div>
        ...
        <!-- <div >
          <p class="font-bold color-0067cc" v-for="(item, ind) in chartsPieOption.series[0].data" :key="ind">{{item.name}}：{{item.type}}</p>
        </div>-->
        <div class="echarts-wrapper">
...
import { orgDataAnalysisHistogram, orgDataanalysisBar, orgDataSelectDetailBySummary } from "@/services/performanceManagement.js";
...
import { GetLecturerlist, drillSubmitApprove, UpdateLecturer, DeleteLecturer, getOrganizeList, DetailLecturer } from "@/services/api.js";
import { type } from "os";
export default {
  mixins: [teableCenterEllipsis, cancelLoading, dataAnalysis],
  components: { Echarts, ReportDataTableVue },
  data() {
    return {
      pushPara: {},
      statusList: [],
      ...
      columns: [
        {
          title: '是否有否决性指标',
          scopedSlots: { customRender: 'rejectType' },
          width: 200,
        },
        {
          title: '安全管理扣分分值',
          dataIndex: 'saftdeductScoreDTOList',
          width: 200,
        },
        {
          title: '消防管理扣分分值',
          dataIndex: 'fireControldeductScoreDTOList',
          width: 200,
        },
        {
          title: '环境管理扣分分值',
          dataIndex: 'environmentdeductScoreDTOList',
          width: 200,
        },
        {
          title: '职业健康管理扣分分值',
          dataIndex: 'occupationdeductScoreDTOList',
          width: 200,
        },
        {
          title: '其他管理事项扣分分值',
          dataIndex: 'othersdeductScoreDTOList',
          width: 200,
        },
        {
          title: '总扣分值',
          dataIndex: 'totaldeductScoreDTOList',
          width: 200,
        },
        {
          title: '总得分',
          dataIndex: 'totalGetPoints',
          width: 200,
        },
        {
          title: '分档',
  ...
  created() {
    this.setRouterCode("performanceCorporation");
    ...
    this.orgDataanalysisBar();
    this.orgDataAnalysisHistogram();
  },
  methods: {
    yearChange(val) {
    ...
    // 查看|编辑 填报数据详情
    orgDataselectDetail() {
      return orgDataSelectDetailBySummary({ ...this.preFormInline }).then(res => {
        let result = res.data || {};
    ...
    iRest: debounce(function () {
      ...
      this.orgDataselectDetail().finally(() => {
        this.cancelLoading();
      })
    ...
    // 饼图
    orgDataanalysisBar() {
      orgDataanalysisBar({ ...this.chartFormInline }).then(res => {
        let pieObj = { ...this.chartsPieOption };
        pieObj.series[0].data = res.data || [];
        this.chartsPieOption = { ...pieObj }
      }).catch(err => { console.log(err) })
    },
    // 柱状图
    orgDataAnalysisHistogram() {
      orgDataAnalysisHistogram({ ...this.chartFormInline }).then(res => {
        let obj = { ...this.chartsBarOption };
        obj.xAxis.data = [];
        obj.series[0].data = [];
        let chartData = res.data || [];
        for (let i = 0; i < chartData.length; i++) {
          obj.xAxis.data.push(this.getMappingValue(this.projectList, "key", chartData[i].xdata).value)
