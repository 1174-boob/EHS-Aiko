      ...
      <template slot="form">
        ...
        >
          ...
          <a-form-model-item class="flex" label="风险分值" prop="riskScore">
            <!-- <a-input type="number"
                                 v-model="editForm.riskScore"
            placeholder="请输入风险分值"></a-input>-->
            <a-input-number v-model="editForm.riskScore" :min="0" :step="0.1" :precision="1" />
          </a-form-model-item>
          <div class="m-b-20">
            <vxe-table
            ...
            >
              ...
              <vxe-column field="deductScore" title="扣分分值" width="180" :edit-render="{autofocus: '.vxe-input--inner', placeholder: '请点击输入扣分分值'}">
                <template #edit="{ row }">
                  <!-- <vxe-input :disabled="editForm.rejectIndex == 1"
                                               v-model="row.deductScore"
                                               placeholder="请点击输入扣分分值"
                  allowClear></vxe-input>-->
...
import { getKpiIndexList, addKpiIndex, deleteKpiIndex, editKpiIndexEdit, checkKpiIndex } from "@/services/performanceManagementBranch.js";
export default {
  ...
  created() {
    this.setRouterCode("performanceBranchIndex");
    this.getDataList();
  },
  computed: {
  },
  methods: {
    ...
    editConfirm() {
      ...
      if (b) {
        this.$antMessage.error("请正确填写扣分标准！");
        return false
      }
      this.editForm.indexItems = this.modalTableData
      this.addLoading = true;
      if (this.isAdd) {
        addKpiIndex(this.editForm).then(() => {
          this.$antMessage.success("新增成功");
          this.editCancle()
          this.getDataList();
        }).catch(err => console.log(err))
      } else {
        editKpiIndexEdit(this.editForm).then(() => {
