<template>
    <HasFixedBottomWrapper>
        ...
        <DashBtn>
            <div>
                <a-button type="dashed"
                          v-if="showSelectDept"
                          @click="selectCorporation">选择部门</a-button>
                <a-button type="dashed"
                          @click="selectIndex">选择指标</a-button>
            </div>
        </DashBtn>
        <PageTitle>
            部门：
            <template v-if="selectDeptList.length">
                <span v-for="(i,index) in selectDeptList"
                      :key="index">{{i.deptName}},</span>
            </template>
            <template v-else><span class="font-14 color-999">请点击上方”选择组织“按钮选择组织</span></template>
        </PageTitle>
        <!-- 启用分值档位 -->
        <div>
            <a-checkbox v-model="startStatus"
                        @change="handleScoreChange">
                启用分值档位
            </a-checkbox>
            <div v-if="startStatus">

                <a-form-model ref="scoreForm"
                              labelAlign="right"
                              :model="scoreFormData"
                                ...
                                <a-row type="flex"
                                    ...
                                    <a-col :span="11">
                                        <a-form-model-item label=""
                                                           prop='levelAOneScore'
                                                           :rules='scoreFormDataRules.aScore[0]'>
                                            <a-input-number v-model="scoreFormData.levelAOneScore"
                                                            placeholder="请输入分值"
                                                            :max='100'
                                    ...
                                    <a-col :span="11">
                                        <a-form-model-item label=""
                                            ...
                                            <a-input-number v-model="scoreFormData.levelATwoScore"
                                                            @change="handleValueChangeAtwo"
                                                            :max='99'
                                                            :min='4'
                                                            placeholder="请输入分值"></a-input-number>
                                        </a-form-model-item>
                                    </a-col>
                                </a-row>
                            </a-form-model-item>
                                ...
                                <a-row type="flex"
                                    ...
                                    <a-col :span="11">
                                        <a-form-model-item label=""
                                                           prop='levelBOneScore'
                                                           :rules='scoreFormDataRules.bScore[0]'>
                                            <a-input-number v-model="scoreFormData.levelBOneScore"
                                                            placeholder="请输入分值"
                                                            @change="handleValueChangeBone"
                                                            :max='levelBOneScoreMax'
                                                            :disabled='true'
                                                            :min='3'></a-input-number>
                                        </a-form-model-item>
                                    </a-col>
                                    <a-col :span="1">
                                        <div style="text-align: center;">-</div>
                                    ...
                                    <a-col :span="11">
                                        <a-form-model-item label=""
                                            ...
                                            <a-input-number v-model="scoreFormData.levelBTwoScore"
                                ...
                                <a-row type="flex"
                                    ...
                                    <a-col :span="11">
                                        <a-form-model-item label=""
                                                           prop='levelCOneScore'
                                                           :rules='scoreFormDataRules.cScore[0]'>
                                            <a-input-number v-model="scoreFormData.levelCOneScore"
                                    ...
                                    <a-col :span="11">
                                        <a-form-model-item label=""
                                            ...
                                            <a-input-number v-model="scoreFormData.levelCTwoScore"
                                                            placeholder="请输入分值"
                                                            :disabled='true'></a-input-number>
                                        </a-form-model-item>
                                    </a-col>
        ...
        <!-- 选择指标 -->
        <CommonModal class="table-modal-large"
                     title="选择指标"
                     :visible="indexVisible"
                     :cancelFn="indexCancle">
            <template slot="form">
                <a-form-model ref="indexForm"
                              :model="indexForm"
                    ...
                    <a-form-model-item class="flex"
                                       label="所属项目"
                                       prop="projectId">
                        <a-select v-model="indexForm.projectId"
                    ...
                    <a-form-model-item class="flex"
                        ...
                        <selectIndex ref="selectIndex"
                                     :selectedRowOld="(indexForm.itemCommandList || [])"></selectIndex>
                    </a-form-model-item>
                </a-form-model>
            </template>
            <template slot="btn">
                ...
                <a-button type="primary"
                          ...
                          @click="indexConfirm">确定</a-button>
            </template>
        </CommonModal>
        <!-- 选择部门 -->
        <CommonModal class="table-modal-large"
            ...
            <div>
                <selectDept ref="selectDept"
                            :selectedRowOld="(selectDeptList || [])"></selectDept>
            </div>
            <template slot="btn">
...
export default {
    ...
    created () {
        if (this.$route.query && this.$route.query.id && this.$route.query.configStatus && this.$route.query.deptId && this.$route.query.deptName) { // 已配置 获取详情
            console.log()
            this.selectDeptList = [
                {
    ...
    methods: {
        ...
        getDetail (e) {
            viewAchDeptConfig({
                ...
                this.bodyIndexData[0].indexList = this.dispose(res.data.safeIndexData)
                this.bodyIndexData[1].indexList = this.dispose(res.data.fireIndexData)
                this.bodyIndexData[2].indexList = this.dispose(res.data.environmentIndexData)
                this.bodyIndexData[3].indexList = this.dispose(res.data.healthIndexData)
                this.bodyIndexData[4].indexList = this.dispose(res.data.otherIndexData)
                this.scoreFormData = {
        ...
        //选择部门
        deptConfirm () {
            this.selectDeptList = this.$refs.selectDept.selectedRow.length ? this.$refs.selectDept.selectedRow : null
        ...
        pageSubmit: debounce(function (n) {
            ...
            console.log(sum)
            if (sum != 100) {
                this.$antMessage.error("所有指标的风险分值合计为100分，不符合是提示“当前所有指标风险分值合计大于/小于100分，请重新配置!");
                return false
            }
        ...
        indexConfirm () {
            ...
            this.bodyIndexData[this.indexForm.projectId].indexList = this.$refs.selectIndex.selectedRow.length ? this.$refs.selectIndex.selectedRow : null
            this.bodyIndexData[this.indexForm.projectId].indexList = this.dispose(this.bodyIndexData[this.indexForm.projectId].indexList)
