...
export default {
  ...
  data() {
    return {
      ...
      tableDataList: [],
      // 导入文件地址
      importAction: window.location.host.indexOf('localhost') < 0 ? `${process.env.VUE_APP_API_PROXY_TARGET}${serviceNameList.ehsEquip}/api/ehs/equip/safeAnnexExcel/importFile` : `/api/ehs/equip/safeAnnexExcel/importFile`,
    }
  },
  created() {
  ...
  methods: {
    ...
    downloadTemplate() {
      window.open(
        window.location.host.indexOf('localhost') < 0 ? `${process.env.VUE_APP_API_PROXY_TARGET}/file/template/安全附件导入模板.xlsx` : `${process.env.VUE_APP_API_BASE_URL}file/template/安全附件导入模板.xlsx`
      )
    },
    getRowClass(row) {
      if (+row.flag === 0) {
