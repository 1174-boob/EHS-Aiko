<template>
  <div class="clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
    <SearchTerm>
      <a-form-model layout="inline" :model="formInline" :colon="false">
        ...
        <a-form-model-item label="设备类型">
          <a-select allowClear show-search v-model="formInline.equipmentType" placeholder="请选择设备类型" option-filter-prop="children" :filter-option="filterOption">
        ...
        <a-form-model-item label="设备状态">
          <a-select allowClear show-search v-model="formInline.equipmentStatus" placeholder="请选择设备状态" option-filter-prop="children" :filter-option="filterOption">
            <a-select-option v-for="item in equipmentStatusOptions" :key="item.key" :value="item.key">{{item.value}}</a-select-option>
          </a-select>
        </a-form-model-item>
        ...
        <a-form-model-item label="登记证编号">
          <a-input allowClear v-model="formInline.useRegistryNum" placeholder="请输入登记证编号"></a-input>
        </a-form-model-item>
        <a-form-model-item label="数据筛选">
          <a-select allowClear show-search v-model="formInline.flag" placeholder="请选择数据筛选" option-filter-prop="children" :filter-option="filterOption">
            <a-select-option v-for="item in flagOptions" :key="item.key" :value="item.key">{{item.value}}</a-select-option>
          </a-select>
        </a-form-model-item>
    ...
    <DashBtn>
      <div>
        <a-button type="dashed" @click="showChooseModal">
          <a-icon type="plus" />新建
        </a-button>
      </div>
      <div>
        <a-button type="primary" class="btn" @click="showImportModal">批量导入</a-button>
    ...
    <CommonTable :spinning="tableSpinning" :page="page" :pageNoChange="pageNoChange" :showSizeChange="onShowSizeChange">
      <a-table bordered :columns="columns" :scroll="{ x: 1000 }" :locale="{emptyText: emptyText}" :data-source="tableDataList" :rowKey="(record, index)=>{return index}" :pagination="false">
        ...
        <div slot="useRegistryNum" slot-scope="record" class="icon-cell">
          <i class="cell-i" :class="getRowClass(record)"></i>
          <span>{{ record.useRegistryNum }}</span>
        </div>
        <div slot="equipmentType" slot-scope="record">{{ record.equipmentType | systemFilter('equipmentType') }}</div>
        <div slot="equipmentStatus" slot-scope="record">{{ record.equipmentStatus | systemFilter('equipmentStatus') }}</div>
    ...
    <ChooseSpecialTypeModal @confirmCallback="confirmCallback" :visible.sync="modalVisible" />
    <ChooseTemplateModal :visible.sync="templateVisible" />
    <ImportModal @successCallback="successCallback" :visible.sync="importModalVisible" />
  </div>
</template>
<script>
...
import ChooseSpecialTypeModal from './comp/ChooseSpecialTypeModal.vue'
import ChooseTemplateModal from './comp/ChooseTemplateModal.vue'
import ImportModal from './comp/ImportModal.vue'
import { debounce, cloneDeep } from 'lodash'
import { specialEquipmentList, specialEquipmentDelete, specialEquipmentExport, specialEquipmentImport, specialEquipmentTemplateDown } from '@/services/specialDevice'
import mixin from '../mixin'
export default {
  components: { ChooseSpecialTypeModal, ChooseTemplateModal, ImportModal },
  ...
  data() {
    return {
      ...
      columns: [
        {
          title: '登记证编号',
          scopedSlots: { customRender: 'useRegistryNum' },
          width: 200
        },
        {
          title: '所属组织',
      ...
      tableDataList: [],
      // 导入文件地址
      // action: `${process.env.VUE_APP_API_BASE_URL}${serviceNameList.danger}/api/ehs/chemical/list/upload`,
    }
  },
  created() {
  ...
  methods: {
    getDataList() {
      ...
      return specialEquipmentList(params)
        .then(res => {
    ...
    deleteItem(row) {
      this.$antConfirm({
        ...
        onOk: async () => {
          ...
          const { code } = await specialEquipmentDelete(para)
          if (+code === 20000) {
    ...
    viewDetail(row) {
      ...
      this.$router.push({
        path: path,
        query: { id: row.specialEquipmentId }
      })
    },
    canUpdate(row) {
    ...
    async exportAll() {
      ...
      let res = await specialEquipmentExport(params)
      if (res) {
    ...
    async downloadTemplate() {
      ...
      // console.log(`${process.env.VUE_APP_API_BASE_URL}/buckets/template/browse/安全附件导入模板.xlsx`, 'file path')
      // window.open(`${process.env.VUE_APP_API_BASE_URL}/buckets/template/browse/安全附件导入模板.xlsx`);
    },
    showImportModal() {
