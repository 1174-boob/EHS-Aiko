  ...
  <div class="searchtable-wrapper clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
    <SearchTerm>
      <a-form-model layout="inline" :model="formInline" :colon="false">
        <CommonDept ref="commonSearchItem" :CommonFormInline="formInline" :hasDepartment="false"></CommonDept>
        <a-form-model-item label="公司名称">
          <a-input v-model.trim="formInline.company" :maxLength="300" placeholder="请输入公司名称" allowClear></a-input>
        </a-form-model-item>
        <a-form-model-item label="姓名">
          <a-input v-model.trim="formInline.userName" :maxLength="20" placeholder="请输入姓名" allowClear></a-input>
        </a-form-model-item>
        <a-form-model-item label="身份证号">
          <a-input v-model.trim="formInline.userCode" :maxLength="100" placeholder="请输入身份证号" allowClear></a-input>
        </a-form-model-item>
    ...
    <CommonTable :spinning="tableSpinning" :page="page" :pageNoChange="pageNoChange" :showSizeChange="showSizeChange">
      <vxe-table class="vxe-scrollbar beauty-scroll-fireBox" border show-header-overflow show-overflow align="center" :row-config="{isHover: true}" :data="tableList">
        ...
        <vxe-column field="phone" :min-width="120" title="手机号"></vxe-column>
        <vxe-column field="userCode" :min-width="140" title="证件号码"></vxe-column>
        <vxe-column field="company" :min-width="120" title="公司名称"></vxe-column>
...
export default {
  ...
  methods: {
    ...
    goDetail(row) {
      let query = { userId: row.userId, companyId: row.companyId, type: '2' }
      this.$router.push({
    ...
    iSearch() {
      ...
      if (this.formInline.userCode && this.formInline.userCode.length < 5) {
        this.$antMessage.warn('身份证号最少输入5位');
        return
      }
      this.handleLoading();
      this.getTableList();
