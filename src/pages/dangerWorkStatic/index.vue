...
export default {
  ...
  data() {
    return {
      ...
      dictionary,
      // 导入文件地址
      action: `${process.env.VUE_APP_API_BASE_URL}${serviceNameList.safe}/api/ehs/safe/danger/importFile`,
      page: {
        pageNo: 1,
        pageSize: 10,
        total: 0,
  ...
  methods: {
    ...
    // 下载模板
    downloadTem() {
      // http://dev.boeplat.com.cn/file/resource/danger.xlsx
      let tarUrl = `${process.env.VUE_APP_API_BASE_URL}/file/resource/danger.xlsx`
      window.open(tarUrl)
    },
    // 批量导出
    exportAll() {
