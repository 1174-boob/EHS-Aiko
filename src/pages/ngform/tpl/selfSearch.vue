<template>
  ...
  <SearchTerm>
    <el-form :inline="true" ref="dataForm" :model="dataForm">
      <!-- <el-form-item>
        <a-select v-model="dataForm.centerArea" placeholder="请选择所属中心" @change="handleChange">
          <a-select-option v-for="item in centerAreaList" :key="item.key" :value="item.key">{{item.value}}</a-select-option>
        </a-select>
      </el-form-item> -->
      <CommonSearchItem :CommonFormInline="dataForm"></CommonSearchItem>
...
export default {
  ...
  data() {
    return {
      ...
      // centerAreaList: dictionary("center_area")
    }
  },
