<template>
  <CommonModal title="模板预览" :visible="tempPreviewModelShow" :cancelFn="closeModel" class="tempPreviewModel">
    <template>
      <div class="pdf-preview-out" v-loading="boxLoading">
        ...
        <div class="scroll-box">
          ...
          <div class="pdf-box">
            ...
            <div :style="getPdfSize()" class="pdf-scale-box">
              <!-- 预览组件 -->
              <pdf :src="pdfUrl" :page="currentPage" @num-pages="getTotalPage" @page-loaded="pageLoaded"
        ...
        <div class="bottom-tools">
          ...
          <div class="page">
            <el-button round type="primary" :disabled="currentPage === 1" @click="chengPage">上一页</el-button>
            <el-button round type="primary" :disabled="currentPage === pageTotal" @click="chengPage('+')">下一页</el-button>
          </div>
          <div class="scale">
            <el-button type="primary" icon="el-icon-minus" circle :disabled="(pageScale - 0.1) < 0.3" @click="scalePage">
            </el-button>
            <el-button type="primary" icon="el-icon-plus" circle :disabled="(pageScale + 0.1) > 1" @click="scalePage('+')"></el-button>
          </div>
        </div>
      </div>
    </template>
    <template slot="btn">
      ...
      <a-button v-if="!readOnly" type="primary" style="margin-left: 0.2rem" @click="onSelect">选择</a-button>
    </template>
  </CommonModal>
</template>
...
export default {
  components: { pdf },
  model: {
    prop: 'tempPreviewModelShow',
  },
  props: {
    tempPreviewModelShow: {
      type: Boolean,
      default: false,
    },
    previewData: {
  ...
  methods: {
    onSelect(){
      this.$emit('selTempFn',this.previewData)
      this.closeModel()
    },  
    // 取消-关闭model
    closeModel() {
    ...
    getPaperUrl() {
      const filePath = this.previewData?.file?.filePath
      if(filePath){
...
.pdf-preview-out {
  ...
  // height: 600px;
  height: calc(100vh - 100px - 55px - 24px - 24px - 56px - 30px - 86px);
