  ...
  methods: {
    ...
    copyCode() {
      ...
      clipboard.on('success', () => {
        this.$antMessage.success(`复制成功，覆盖文件 src/config/config.js 然后重启项目即可生效`).then(() => {
          ...
          if (localConfig) {
            ...
            this.$antMessage.warn('检测到本地有历史保存的主题配置，想要要拷贝的配置代码生效，您可能需要先重置配置', 5)
          }
        })
        clipboard.destroy()
      })
    ...
    saveSetting() {
      const closeMessage = this.$antMessage.loading('正在保存到本地，请稍后...', 0)
