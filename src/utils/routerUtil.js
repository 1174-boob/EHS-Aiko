...
function parseRoutes(routesConfig, routerMap) {
  ...
  routesConfig.forEach(item => {
    ...
    if (typeof item === 'string') {
      ...
      // if(item.router == 'preview') {
      //   router = routerMap[item.router]
      // }
      router = routerMap[item.router] || routerMap[item.url]
      routeCfg = item
    }
    if (!router) {
    ...
    const route = {
      path: routeCfg.path || router.path || routeCfg.router || routeCfg.url || '404',
      name: routeCfg.name || router.name || routeCfg.resourceName,
      component: router.component,
      redirect: routeCfg.redirect || router.redirect,
      meta: { ...meta, authority: meta.authority || '*' }
    }
    if (routeCfg.invisible || router.invisible || route.path === '404' || (!routerMap[route.path] && route.path !== 'preview')) {
 ...
 */
function loadRoutes(routesConfig, nextPage) {
  ...
  if (asyncRoutes) {
    if (routesConfig && routesConfig.length > 0) {
      ...
      console.log('addRoutes start', finalRoutes)
  ...
  if (menuRoutes) {
    ...
    sessionStorage.setItem("menuData", JSON.stringify(menuRoutes));
  }
}
